        ...
        if ((in_array($fileMimeType, $allowedImageTypes))) {
            ...
            if ($fileSize <= $maxFileSize) {
                ...
                if (move_uploaded_file($profile_pic['tmp_name'], $filePath)) {
                    $stmt = $conn->prepare('INSERT INTO vice_rector (name,profile_pic,responsibility,bio,respect) VALUES (?, ?,?,?,?)');
                    $stmt->execute([$fullname,$filePath,$res,$bio,$respect]);
                    echo "<p style='color:green;'>Vice Rector uploaded successfully!</p>";
                } else {
                    echo "<p style='color:red;'>Vice Rector upload failed!</p>";
                }
            } else {
    ...
    <div class="admin-container">
        ...
        <main class="content">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <button type="submit" name="upload" id="upload">Create Vice Rector</button>
            </form>
            </section>
          </main>
    </div>
