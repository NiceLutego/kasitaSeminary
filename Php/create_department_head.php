...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'], $_POST['pos'],$_POST['department'],$_POST['bio'],$_POST['respect'])){
    $fullname = $_POST['name'] ?? '';
    $position = $_POST['pos'] ?? '';
        ...
        if ((in_array($fileMimeType, $allowedImageTypes))) {
            ...
            if ($fileSize <= $maxFileSize) {
                ...
                if (move_uploaded_file($profile_pic['tmp_name'], $filePath)) {
                    $stmt = $conn->prepare('INSERT INTO department_heads (name,profile_pic,position,department,bio,respect) VALUES (?, ?,?,?,?,?)');
                    $stmt->execute([$fullname,$filePath,$position,$department,$bio,$respect]);
                    echo "<p style='color:green;'>Department Head uploaded successfully!</p>";
                } else {
                    echo "<p style='color:red;'>Department Head upload failed!</p>";
                }
            } else {
    ...
    <div class="admin-container">
        ...
        <main class="content">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <button type="submit" name="upload" id="upload">Create Department Head</button>
            </form>
            </section>
          </main>
    </div>
