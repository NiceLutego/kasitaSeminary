...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'], $_POST['pos'],$_POST['bio'],$_POST['respect'])){
    $fullname = $_POST['name'] ?? '';
    $position = $_POST['pos'] ?? '';
    $bio = $_POST['bio'] ?? '';
        ...
        if ((in_array($fileMimeType, $allowedImageTypes))) {
            ...
            if ($fileSize <= $maxFileSize) {
                ...
                if (move_uploaded_file($profile_pic['tmp_name'], $filePath)) {
                    $stmt = $conn->prepare('INSERT INTO training_mentor (name,profile_pic,position,bio,respect) VALUES (?, ?,?,?,?)');
                    $stmt->execute([$fullname,$filePath,$position,$bio,$respect]);
                    echo "<p style='color:green;'>Training mentor uploaded successfully!</p>";
                } else {
                    echo "<p style='color:red;'>Training Mentor upload failed!</p>";
                }
            } else {
...
<body>
    <div class="admin-container">
        ...
        <main class="content">
            <header>
                <h1>Create Create Training Mentor</h1>
            </header>
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <button type="submit" name="upload" id="upload">Create Training Mentor</button>
            </form>
            </section>
          </main>
    </div>
