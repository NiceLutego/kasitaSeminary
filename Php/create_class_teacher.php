...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'],$_POST['upload'], $_POST['pos'],$_POST['class'],$_POST['bio'],$_POST['respect'])){
    $fullname = $_POST['name'] ?? '';
    $position = $_POST['pos'] ?? '';
    $class = $_POST['class'] ?? '';
    $bio = $_POST['bio'] ?? '';
        ...
        if ((in_array($fileMimeType, $allowedImageTypes))) {
            ...
            if ($fileSize <= $maxFileSize) {
                ...
                if (move_uploaded_file($profile_pic['tmp_name'], $filePath)) {
                    $stmt = $conn->prepare('INSERT INTO class_teachers (name,profile_pic,position,class,bio,respect) VALUES (?, ?,?,?,?,?)');
                    $stmt->execute([$fullname,$filePath,$position,$class,$bio,$respect]);
                    echo "<p style='color:green;'>Class Teacher uploaded successfully!</p>";
                } else {
                    echo "<p style='color:red;'>Class Teacher upload failed!</p>";
                }
            } else {
        ...
        <main class="content">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <select name="class" id="class">
                  <option value="Form IA">Form I A</option>
                  <option value="Form IB">Form I B</option>
                  <option value="Form IIA">Form II A</option>
                  <option value="Form IIB">Form II B</option>
                ...
                <button type="submit" name="upload" id="upload">Create Class Teacher</button>
            </form>
            </section>
          </main>
    </div>
