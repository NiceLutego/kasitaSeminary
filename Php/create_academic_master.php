        ...
        if ((in_array($fileMimeType, $allowedImageTypes))) {
            ...
            if ($fileSize <= $maxFileSize) {
                ...
                if (move_uploaded_file($profile_pic['tmp_name'], $filePath)) {
                    $stmt = $conn->prepare('INSERT INTO academic_masters (name,profile_pic,position,bio,respect) VALUES (?, ?,?,?,?)');
                    $stmt->execute([$fullname,$filePath,$position,$bio,$respect]);
                    echo "<p style='color:green;'>Academic Master uploaded successfully!</p>";
                } else {
                    echo "<p style='color:red;'>Academic Master upload failed!</p>";
                }
            } else {
    ...
    <div class="admin-container">
        ...
        <main class="content">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <button type="submit" name="upload" id="upload">Create Academic</button>
            </form>
            </section>
          </main>
    </div>
