<?php
    ...
    if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['name'],$_POST['position'],$_POST['bio'],$_POST['res'])){
      $name = $_POST['name'];
          ...
          if ((in_array($fileMimeType, $allowedImageTypes))) {
              ...
              if ($fileSize <= $maxFileSize) {
                  ...
                  if (move_uploaded_file($profile_pic['tmp_name'], $filePath)) {
                      $stmt = $conn->prepare('INSERT INTO management_team (name, position,profile_pic,responsibility,bio) VALUES (?, ?,?,?,?)');
                      $stmt->execute([$name,$position, $filePath,$res,$bio]);
                      echo "<p style='color:green;'>Member uploaded successfully!</p>";
                  } else {
                      echo "<p style='color:red;'>Member upload failed!</p>";
                  }
              } else {
...
<body>
    <div class="admin-container">
        ...
        <main class="content">
            <header>
                <h1>Upload Member of Management Team</h1>
            </header>
            <form action="" method="post" enctype="multipart/form-data">
              ...
              <input type="file" name="profile-pic" id="profile-pic" required><br><br>
