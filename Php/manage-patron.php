<?php
    ...
    if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['remove'])){
      ...
      if(!empty($id)){
        $stmt = $conn ->prepare("DELETE FROM patron WHERE id = ?");
        $stmt ->bind_param("i",$id);
        if($stmt ->execute()){
          echo "<p style='color:green;'>The The teacher is deleted successful.</p>";
        }
        else{
    ...
    $result = $conn ->query("SELECT * FROM patron");
    ?>
...
<body>
    <div class="admin-container">
        ...
        <main class="content">
            <header>
                <h1>Student Patrons</h1>
            </header>
            <h3>Kasita Seminary Student Welfare</h3>
            <table border="1">
              <tr>
                <th>Name</th>
              ...
              <tr>
                  ...
                  <td><?=htmlspecialchars($row['Domitory']);?></td>
            ...
            <form action="../Php/create_patron.php" method="post">
