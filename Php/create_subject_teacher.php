...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'],$_POST['upload'], $_POST['pos'],$_POST['subjects'],$_POST['bio'],$_POST['respect'])){
    $fullname = $_POST['name'] ?? '';
    $position = $_POST['pos'] ?? '';
        ...
        if ((in_array($fileMimeType, $allowedImageTypes))) {
            ...
            if ($fileSize <= $maxFileSize) {
                ...
                if (move_uploaded_file($profile_pic['tmp_name'], $filePath)) {
                    $stmt = $conn->prepare('INSERT INTO subject_teachers (name,profile_pic,position,subject,bio,respect) VALUES (?, ?,?,?,?,?)');
                    $stmt->execute([$fullname,$filePath,$position,$subject,$bio,$respect]);
                    echo "<p style='color:green;'>Subject Teacher uploaded successfully!</p>";
                } else {
                    echo "<p style='color:red;'>Subject Teacher upload failed!</p>";
                }
            } else {
...
<body>
    <div class="admin-container">
        ...
        <main class="content">
            <header>
                <h1>Create Subjects Teachers</h1>
            </header>
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <label for="department">Subjects:</label>
                ...
                <button type="submit" name="upload" id="upload">Create Subjects Teacher</button>
            </form>
            </section>
          </main>
    </div>
