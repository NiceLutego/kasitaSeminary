<?php
    ...
    if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['remove'])){
      ...
      if(!empty($id)){
        $stmt = $conn ->prepare("DELETE FROM production_teachers WHERE id = ?");
        $stmt ->bind_param("i",$id);
        if($stmt ->execute()){
          echo "<p style='color:green;'>The The Master is deleted successful.</p>";
        }
        else{
    ...
    $result = $conn ->query("SELECT * FROM production_teachers");
    ?>
...
<body>
    <div class="admin-container">
        ...
        <main class="content">
            ...
            <h3>Kasita Seminary Production Teacher</h3>
            <table border="1">
              <tr>
                <th>Name</th>
            ...
            <form action="../Php/create_production_teacher.php" method="post">
              <button type="submit">Create new Master</button>
            </form>
