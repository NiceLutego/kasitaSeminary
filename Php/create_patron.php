...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['name'],$_POST['upload'], $_POST['pos'],$_POST['dom'],$_POST['bio'],$_POST['respect'])){
    $fullname = $_POST['name'] ?? '';
    $position = $_POST['pos'] ?? '';
    $dom = $_POST['dom'] ?? '';
    $bio = $_POST['bio'] ?? '';
        ...
        if ((in_array($fileMimeType, $allowedImageTypes))) {
            ...
            if ($fileSize <= $maxFileSize) {
                ...
                if (move_uploaded_file($profile_pic['tmp_name'], $filePath)) {
                    $stmt = $conn->prepare('INSERT INTO patron (name,profile_pic,position,domitory,bio,respect) VALUES (?, ?,?,?,?,?)');
                    $stmt->execute([$fullname,$filePath,$position,$dom,$bio,$respect]);
                    echo "<p style='color:green;'>Patron uploaded successfully!</p>";
                } else {
                    echo "<p style='color:red;'>Patron upload failed!</p>";
                }
            } else {
    ...
    <div class="admin-container">
        ...
        <main class="content">
            ...
            <form action="" method="post" enctype="multipart/form-data">
                ...
                <label for="Domitory">Domitory:</label>
                <input type="text" name="dom" id="dom"><br><br>
                <label for="profile-pic">Profile Picture:</label>
                <input type="file" name="profile-pic" id="profile-pic"><br><br>
                <label for="bio">Bio:</label>
                <input type="text" name="bio" id="bio"><br><br>
                <button type="submit" name="upload" id="upload">Create Patron</button>
            </form>
            </section>
          </main>
    </div>
